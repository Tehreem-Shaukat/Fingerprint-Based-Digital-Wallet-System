...
app.post('/api/wallet/send', async (req, res) => {
    try {
        ...
        console.log(`Transfer request: ${sender} → ${receiver}, amount: ${parsedAmount}`);

        // Delegate to Postgres function so both balances update atomically
        ...
        if (error) {
            console.error('Supabase RPC error (transfer_funds):', error);
        ...
        if (!result || !result.success) {
            return res.status(400).json({
                error: result ? result.message : 'Transfer failed'
            });
        }
        ...
        console.error('Transfer endpoint error:', err);
        return res.status(500).json({
